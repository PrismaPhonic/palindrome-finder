...
(let* ((here (uiop:pathname-directory-pathname *load-truename*))
       (runner "runner-comm-smallest-inner.lisp")
       (out    #P"../target-bin/palprod-comm-smallest-inner"))
  (uiop:chdir here)
  ...
  (let ((pkg (find-package :pp-runner-comm-smallest-inner)))
    (unless pkg
      (error "Runner package PP-RUNNER-COMM-SMALLEST-INNER not found."))
    (let* ((main-sym (intern "MAIN" pkg))
           (main-fn  (and (fboundp main-sym) (symbol-function main-sym))))
      (unless main-fn
        (error "Symbol MAIN not fbound in PP-RUNNER-COMM-SMALLEST-INNER."))
