(defpackage :pp-runner-comm-smallest-inner
  (:use :cl)
  (:export :main))
(in-package :pp-runner-comm-smallest-inner)
...
(defun %do-iters (min max iters)
  ...
  (loop repeat iters do (pp-comm:smallest-inner min max))
  (multiple-value-bind (p _vec) (pp-comm:smallest-inner min max)
    (declare (ignore _vec))
    p))
