;; Ensures that fast.lisp and the testing library are always loaded
...
(test find-the-smallest-palindrome-from-single-digit-factors
      (let ((min-factor 1)
            ...
            (palindrome 1)
            (factors '((1 1))))
        (is (multiple-value-bind (p f) (pp-fast:smallest min-factor max-factor)
...
(test find-the-largest-palindrome-from-single-digit-factors
      (let ((min-factor 1)
            ...
            (palindrome 9)
            (factors '((1 9) (3 3))))
        (is (multiple-value-bind (p f) (pp-fast:largest min-factor max-factor)
              (and (eql p palindrome) (equal f factors))))))
...
(test error-result-for-smallest-if-min-is-more-than-max
      (let ((min-factor 10000)
            (max-factor 1))
        (is (eql NIL (pp-fast:smallest min-factor max-factor)))))

(test error-result-for-largest-if-min-is-more-than-max
      (let ((min-factor 2)
            (max-factor 1))
        (is (eql NIL (pp-fast:largest min-factor max-factor)))))
