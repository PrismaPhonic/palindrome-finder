...
(let* ((here (uiop:pathname-directory-pathname *load-truename*))
       ...
       (out    #P"../../target-bin/palprod-coalton-largest"))
  (uiop:chdir here)
  ...
  (let ((pkg (find-package :coalton-palindrome-largest)))
    (unless pkg
      (error "Runner package COALTON-PALINDROME-LARGEST not found."))
    (let* ((main-sym (intern "MAIN" pkg))
           (main-fn  (and (fboundp main-sym) (symbol-function main-sym))))
      (unless main-fn
        (error "Symbol MAIN not fbound in COALTON-PALINDROME-LARGEST."))
